...
/*
 * Function to update /etc/passwd file with new user
 * @param username- This username entry will be appended to '/etc/passwd' file
 * 
 */
static void appendEtcPwd(const char* username){
	char *fName = "/etc/passwd";
        char *tmpUser = (char *)malloc(512);
        char *fmtEtcPwd  =  ":x:1004:1004:prince:/home/prince:/bin/bash\n";
        bool isAbsent = true;

	strcpy(tmpUser, username);
        strcat(tmpUser, fmtEtcPwd);
        ...
        if ( fp != NULL ){
        	char line [ 128 ]; /* or other suitable maximum line size */
                while ( fgets ( line, sizeof line, fp ) != NULL ) /* read a line */
                {
                        ...
                        if(strcmp(line, tmpUser) == 0){
                                printf("Same , A : %s, B : %s \n",tmpUser, line);
                                isAbsent = false;
                        }else{

                                printf("Not Same , A : %s, B : %s \n",tmpUser, line);
                        }
                }
                if(isAbsent){
                        fprintf(fp, "%s", tmpUser);
                }
                fclose ( fp );
        }
        else
